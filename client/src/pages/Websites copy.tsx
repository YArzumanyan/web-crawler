...
import { startTask, stopTask } from "../graphql/website";

const Websites: React.FC = () => {
  const [websites] = useState<WebsiteWithNodes[]>(
    useLoaderData() as WebsiteWithNodes[]
  );
  const [rows, setRows] = useState<Record<string, Website>>({});
  const [graphView, setGraphView] = useState<boolean>(false);
  const apiRef = useGridApiRef();

  useEffect(() => {
    const dict: Record<string, WebsiteWithNodes> = {};
