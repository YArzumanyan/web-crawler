...
class DatabaseEntry {
  ...
  async saveCrawlRecord(record: CrawlRecord): Promise<string> {
    ...
    for (const record of allRecords) {
      ...
      await AppDataSource.getRepository(CrawlRecord).update(record.id!, {
        matchLinksRecordIds: record.matchLinksRecordIds,
        owner: owner,
      });
    }
      ...
      if (next) {
        ...
        for (const childId of next.matchLinksRecordIds) {
          if (!allFoundUrls[childId]) {
            const child = await crawlRecordRepository.findOneBy({
              id: childId,
            });
            if (child) {
              queue.push(child);
  ...
  async updateWebsite(website: Website): Promise<Website | null> {
    this.#mutex.acquire();
    const websiteRepository = AppDataSource.getRepository(Website);
    const updateResult = await websiteRepository.update(website.id!, website);
    this.#mutex.release();
    return updateResult ? website : null;
  }
  ...
  async getWebsiteCrawlRecords(websiteId: string[]): Promise<CrawlRecord[]> {
    this.#mutex.acquire();
    const crawlRecordRepository = AppDataSource.getRepository(CrawlRecord);
    const crawlRecords = await crawlRecordRepository.find({
      where: {
        owner: {
          id: In(websiteId),
        },
  ...
  async removeWebsiteCrawlRecords(websiteId: string): Promise<boolean> {
    ...
    const response = await Promise.all(
      nodeIds.map((nodeId) => crawlRecordRepository.remove(nodeId))
    );
    this.#mutex.release();
